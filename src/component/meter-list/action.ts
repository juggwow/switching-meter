...
export async function fetchMeters({
  ...
  try {
    console.log("Incoming filter:", filter, "Page:", page, "PageSize:", pageSize);
    ...
    if (filter?.status) {
        if (filter.status === "wait_installation") {
            ...
            if (filter.pickerDateEnd && filter.pickerDateEnd.getTime() > endOfYesterday.getTime()) {
    ...
    if (filter?.pickerDateStart || finalPickerDateEnd) { // ใช้ finalPickerDateEnd ที่นี่
        where = {
            ...where,
            pickerDate: {
